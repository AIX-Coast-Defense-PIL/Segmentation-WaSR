...
def check_validity_load_results(args, logger):
    ...
    test_path_dict_value = list(paths_dict.values())[0]
    for path_value in paths_dict.values():
        assert sorted(test_path_dict_value) == sorted(path_value)

    # Get File Names (without extension)
    file_names = [os.path.splitext(fn)[0] for fn in test_path_dict_value]
...
if __name__ == "__main__":
    ...
    for idx, (fname, masks_dict) in enumerate(mask_results_dict.items()):
        ...
        # Declare Plt Figure
        fig_mask = plt.figure(dpi=150)
        fig_mask.suptitle(fname, fontsize=20)
        ...
        for abl_idx in range(ablation_number):
            ...
            mask_src_name = mask_src_path.split("Segmentation-WaSR/WaSR/")[-1].split("/")[0]

            # Set Axes
            curr_axes = fig_mask.add_subplot(1, ablation_number, abl_idx+1)
            curr_axes.grid(False)
            curr_axes.set_xticks([])
            curr_axes.set_yticks([])
            curr_axes.set_title(mask_src_name)
        ...
        # Save Figure
        fig_mask.savefig(
            os.path.join(result_dir, "{}.pdf".format(fname)),
        )
